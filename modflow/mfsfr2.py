...
class ModflowSfr2(Package):
    ...
    def load(f, model, nper=None, gwt=False, nsol=1, ext_unit_dict=None):
        ...
        while True:
            ...
            if line[0] != '#':
                break
        # Item 1
        if 'options' in line.lower(): # this should be supported for mf2005 and mfnwt
            line = next(f)
            while 'end' not in line.lower():
                if "reachinput" in line.lower():
                    ...
                    raise ValueError('unexpected/unsupported option for SFR package dataset 1a: {}'.format(line))
            line = next(f)
        else:
            if "reachinput" in line.lower():
                """
    ...
    def write_file(self, filename=None):
        ...
        # Item 1
        if self.parent.version.lower() == 'mfnwt':  # mfnwt does not support options on one line
            if self.reachinput or self.transroute or self.tabfiles:
                f_sfr.write('options\n')
                if self.reachinput:
                    """
                    When REACHINPUT is specified, variable ISFROPT is read in data set 1c.
